...
public:
  WatcherListener(FilesystemWatcher::Callback callback)
      : m_callback(callback) {}
  ...
  void handleFileAction(efsw::WatchID watchid, const std::string &dir,
    ...
    case efsw::Actions::Delete:
      e = WatchEvent::Deleted;
      break;
    case efsw::Actions::Modified:
      e = WatchEvent::Modified;
      break;
...
void FilesystemWatcher::start() {
  ...
  try {
    m_impl->watchId =
        m_impl->watcher.addWatch(m_path, m_impl->listener.get(), true);
    if (m_impl->watchId < 0) {
    ...
    m_impl->running = true;
    std::cout << "[Watcher] Started monitoring: " << m_path << " (efsw)"
              << std::endl;
