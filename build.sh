...
if [[ "$OSTYPE" == "darwin"* ]] || [[ "$OSTYPE" == "linux-gnu"* ]]; then
    ...
    if [ -n "$VCPKG_ROOT" ] && [ -f "$VCPKG_ROOT/scripts/buildsystems/vcpkg.cmake" ]; then
        ...
        # Detect architecture for macOS triplet
        if [[ "$OSTYPE" == "darwin"* ]]; then
            ARCH=$(uname -m)
            VCPKG_TRIPLET="-DVCPKG_TARGET_TRIPLET=$ARCH-osx"
            echo "Using vcpkg toolchain from $VCPKG_ROOT ($ARCH-osx)"
        else
...
if [ ! -f build/CMakeCache.txt ] || [ CMakeLists.txt -nt build/CMakeCache.txt ]; then
    ...
    cmake -S . -B build -DCMAKE_EXPORT_COMPILE_COMMANDS=ON $VCPKG_TOOLCHAIN $VCPKG_TRIPLET
