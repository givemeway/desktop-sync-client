...
// windows.h/winnt.h defines these as macros; they corrupt our enum parsing.
#ifdef DELETE
#  undef DELETE
#endif
#ifdef ERROR
...
//   2. start()             — connects callbacks, begins serving hydration requests
//   3. stop()              — disconnects callbacks, frees CF connection
//
// Integration with CloudSyncWorker:
//   - Call createFilePlaceholder()  instead of downloading in processFilesToDownload()
//   - Call createDirPlaceholder()   instead of fs::create_directories() in processFoldersToCreate()
...
private:
    ...
    static void CALLBACK onNotifyFileOpened(
        const CF_CALLBACK_INFO       *info,
        const CF_CALLBACK_PARAMETERS *params);

    static void CALLBACK onNotifyFileClosed(
        const CF_CALLBACK_INFO       *info,
        const CF_CALLBACK_PARAMETERS *params);
    ...
    CF_PLACEHOLDER_CREATE_INFO buildFilePlaceholderInfo(
        const CloudFileMetadata &file) const;

    // Build a CF_PLACEHOLDER_CREATE_INFO for a directory.
    CF_PLACEHOLDER_CREATE_INFO buildDirPlaceholderInfo(
